...
// Category-specific CO₂ savings (kg per item kept out of landfill)
...
export async function POST(req: Request) {
  try {
    ...
    const [ecoResult, fraudResult, descResult] = await Promise.all([

      // Agent 1: Eco scoring
      anthropic.messages.create({
        model:      "claude-sonnet-4-20250514",
        max_tokens: 200,
        system: `You are an eco-impact scoring agent. Score items 1-100 based on environmental benefit of reuse.
Consider: category CO₂ baseline (${co2Base}kg), condition, description quality, and reuse potential.
Respond ONLY with valid JSON: {"score": number, "reason": "one sentence", "co2_saved_kg": number}`,
      ...
      anthropic.messages.create({
        model:      "claude-sonnet-4-20250514",
        max_tokens: 200,
        system: `You are a fraud detection agent for a swap/donate marketplace.
...
Respond ONLY with valid JSON: {"flagged": boolean, "reason": "one sentence or null"}`,
      ...
      anthropic.messages.create({
        model:      "claude-sonnet-4-20250514",
        max_tokens: 300,
        system: `You are a listing description writer for an eco-swap marketplace.
Given rough notes or a basic description, write a clear, friendly, 2-3 sentence product description.
Highlight condition, what it's good for, and why it's worth swapping.
Respond ONLY with the improved description text — no JSON, no preamble.`,
    ...
    // Parse results
    const ecoText   = ecoResult.content.find((b) => b.type === "text")?.text ?? "{}";
    const fraudText = fraudResult.content.find((b) => b.type === "text")?.text ?? "{}";
    const improvedDesc = descResult.content.find((b) => b.type === "text")?.text ?? listing.description;
