...
export async function POST(req: Request) {
  try {
    ...
    const result = await anthropic.messages.create({
      ...
      max_tokens: 400,
      system: `You are a swap matching agent. Given a user's listings and available listings from others,
find the 3 best matches the user might want to swap for.
Consider: category relevance, complementary items, condition.
Respond ONLY with valid JSON: {"matches": [{"id": "listing_id", "reason": "one sentence why this is a good match"}]}`,
      messages: [{
        role: "user",
        content: `MY LISTINGS:\n${JSON.stringify(myListings, null, 2)}\n\nAVAILABLE LISTINGS:\n${JSON.stringify(otherListings.map(l => ({ id: l.id, title: l.title, category: l.category, condition: l.condition })), null, 2)}`,
      }],
    });
